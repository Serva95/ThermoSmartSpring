        ...
        for (OrariOnOffStringDTO e : orari.getOrariOnOffs()) {
            ...
            try {
                ...
                if(tmp.getOrarioAccensioneA().isAfter(tmp.getOrarioSpegnimentoA()))
                    throw new InvalidFieldException("L'orario di accensione non può essere successivo a quello di spegnimento, riprova.");
                if (!(e.getOrarioAccensioneB().equals("") || e.getOrarioSpegnimentoB().equals(""))) {
                    ...
                    if(tmp.getOrarioSpegnimentoA().isAfter(tmp.getOrarioAccensioneB()))
                        throw new InvalidFieldException("L'orario di accensione di una fascia deve essere successivo a quello di spegnimento della fascia precedente, riprova.");
